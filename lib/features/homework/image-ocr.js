 ...
 * GOAT Bot 2.0
 * Updated: 2025-08-24 10:28:00 UTC
...
function parseCredentialsFromEnv() {
  ...
  //    - a path to a file (most common)
  //    - a raw JSON string (less common)
...
function initVisionClientIfNeeded() {
  ...
  try {
    ...
    // If no explicit creds found, attempt ADC (works on GCP environments)
    console.log(
...
async function processHomeworkImage(imageData, userId, attempt = 0) {
  ...
  try {
    // Ensure Vision client exists (with robust env handling)
    ...
    if (cachedResult) {
      ...
      // textDetection returns [result, metadata]; normalize
