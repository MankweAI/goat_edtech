 ...
 * GOAT Bot 2.0
 * Updated: 2025-08-24 11:56:00 UTC
...
class ConsolidatedHomeworkHelp {
      ...
      if (text) {
        ...
        if (nav) {
          ...
          if (nav.action === "go_exam") {
            // Soft handoff message (routing is handled at /api/index level for new requests)
          ...
          if (nav.action === "go_homework") {
            user.current_menu = "homework_help";
            // fall through to standard homework response below
          }
        }

        // 2) If user types a new question at any time, accept it
        ...
        // 3) Continue flow if already in question context
        if (
  ...
  async handleTextInput(user, text, res) {
    ...
    // Retained for legacy numbered-selection path
    if (user.context?.state === HOMEWORK_STATES.QUESTIONS_DETECTED) {
      const questionNumber = parseInt(text);
      if (questionNumber && user.context.questions[questionNumber - 1]) {
        user.context.selectedQuestion =
    ...
    // New: treat typed questions as direct questions
    if (this.isHomeworkQuestion(lowerText) || isDirectQuestion(lowerText)) {
      const questionType = this.classifyQuestion(text);
  ...
  // Existing intent classification unchanged
