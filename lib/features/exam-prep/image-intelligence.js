...
class ExamPrepImageIntelligence {
  ...
  refineWithClassifier(text, initial) {
    ...
    const map = {
      ...
      series_factorial: {
        ...
        struggle: "factorial telescoping",
      },
      coordinate_geometry_3d: {
        topic: "3D coordinate geometry",
        struggle: "section formula with coordinate planes",
      },
    ...
    if (mapped) {
      ...
      topicConf = Math.max(topicConf || 0.5, 0.85);
  ...
  detectTopic(content) {
    ...
    // NEW: factorial/series patterns
    if (
      /[0-9]+\s*\/\s*[0-9]+\s*!|\bn!\b|∑|sigma|series|show that.*=/.test(text)
    ) {
      return { topic: "series and sequences", confidence: 0.85 };
    }

    // NEW: 3D coordinates + planes
    if (
      /(zx|xy|yz)[-\s]?plane|x\s*=\s*0|y\s*=\s*0|z\s*=\s*0/.test(text) ||
      /\((-?\d+)\s*,\s*(-?\d+)\s*,\s*(-?\d+)\)/.test(text)
    ) {
      return { topic: "3D coordinate geometry", confidence: 0.85 };
    }
