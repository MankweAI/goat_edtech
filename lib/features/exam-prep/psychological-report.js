...
${relatedLine}`;
  ...
  getStruggleStatistics(topic) {
    ...
    for (const key in STRUGGLE_STATISTICS) {
      if (normalizedTopic.includes(key.replace(/_/g, " "))) {
        return STRUGGLE_STATISTICS[key];
      }
    }
  ...
  analyzeEffortFromImage(intelligence, imageAnalysis) {
    ...
    if (organized)
      return "Your organised steps show intent — we’ll sharpen it.";
  ...
  generateSpecificSolution(intelligence) {
    const solutions = {
      ...
      "series and sequences":
        "the telescoping trick with factorial shift (k/(k+1)! simplification)",
      "3d coordinate geometry":
        "section formula + plane condition (x=0/y=0/z=0) checklist",
      // NEW
      "similar triangles":
        "the AA similarity checklist → set up corresponding side ratios → propagate to x and area ratio (k²)",
    };
    const topic = (intelligence.topic || "").toLowerCase();
  ...
  getRelatedPainPoints(intelligence) {
    ...
    return unique; // allow topicDefaults to fill when empty
  }
  ...
  topicDefaults(topic) {
    ...
    const defaults = {
      ...
      derivatives: ["power rule", "product/chain rule", "notation errors"],
      statistics: ["mean/median/mode", "outliers", "units"],
      probability: ["set notation", "Venn diagrams", "conditional reasoning"],
      "series and sequences": [
        "index shift",
        "telescoping sums",
      ...
      "similar triangles": [
        "AA similarity test",
        "corresponding sides order",
        "area ratio equals (scale factor)²",
      ],
      default: ["equation setup", "method steps", "calculation slips"],
    };

    for (const key in defaults) {
      if (t.includes(key.replace(/_/g, " "))) {
        return defaults[key];
      }
    }

    return defaults[t] || defaults.default;
  }
