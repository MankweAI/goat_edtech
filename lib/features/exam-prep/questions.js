...
async function generateExamQuestions(profile, count = 3) {
  ...
  try {
    ...
    // NEW: Apply difficulty adjustments
    const adjustedQuestion = userId
      ? adjustQuestionDifficulty(mainQuestion, enhancedProfile)
      : mainQuestion;

    // Check if the question needs LaTeX rendering - unchanged
    ...
    // Check if solution needs LaTeX rendering
...
function generateContentId(subject, topic) {
  ...
  const subjectCode = subject ? 
    subject.substring(0, 3).toLowerCase() : "gen";
  const topicCode = topic ? 
    topic.substring(0, 3).toLowerCase() : "gen";
