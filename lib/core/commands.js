 ...
 *
 * Change:
 * - CRITICAL: Preserve Exam/Test flow for images when user is in exam_prep_conversation.
 *   Previously any image routed to HOMEWORK_UPLOAD, hijacking the exam flow.
 */
...
function parseGoatCommand(message, userContext, attachments = {}) {
  ...
  if (
    ...
    if (
      !text.includes("test") ||
      (text === "test" && userContext.current_menu !== "exam_prep_conversation")
    ) {
  ...
  // CRITICAL FIX: Preserve exam-prep flow for images when in exam mode
  if (imageInfo && (imageInfo.type === "direct" || imageInfo.type === "url")) {
    if (userContext.current_menu === "exam_prep_conversation") {
      return {
    ...
    // Otherwise route to homework (existing behavior)
    return {
  ...
  if (
    /^[123]$/.test(text) &&
    (!userContext.current_menu || userContext.current_menu === "welcome")
  ) {
    return {
