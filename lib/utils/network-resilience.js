...
function queueForRetry(type, ...args) {
  ...
  const keyString = typeof args[0] === 'string' ? args[0] : JSON.stringify(args[0]);
  const key = `${type}:${keyString}`;
  const retryCount = retryCounters.get(key) || 0;
  
  // CRITICAL FIX: Increase max retries for fetch failures
  const maxRetries = type === 'state' ? 5 : MAX_RETRIES; // More retries for state operations
  
  if (retryCount >= maxRetries) {
    console.log(`⚠️ Max retries (${maxRetries}) reached for ${type} operation: ${key}`);
  ...
  if (retryQueues[type].length < 100) { // Increased queue size for fetch failures
    ...
    console.log(`🔄 Queued ${type} operation for retry: ${key} (attempt ${retryCount + 1}, delay: ${Math.round(delay/1000)}s)`);
  }
}
