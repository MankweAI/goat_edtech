...
// Add a text-only fallback renderer for when both canvas and MathJax are unavailable
async function renderTextOnly(latex, options = {}) {
  console.log('📄 Using text-only LaTeX fallback');
...
async function renderLatexImage(latex, options = {}) {
  ...
  try {
    // Try different rendering methods in order
    let result;
    ...
    if (mjAPI) {
      try {
        ...
        console.error("❌ MathJax rendering failed, trying fallback:", mjError);
      }
    }
    ...
    if (!result && canvas) {
      try {
        ...
        console.error(
          "❌ Canvas rendering failed, using text-only:",
          canvasError
        );
      }
    }
    ...
    if (!result) {
      result = await renderTextOnly(latex, options);
    }
